# 1.3 完整 Tokenizer 类的封装

> [!NOTE]
> **本节目标**
> - 实现完整的 `BPETokenizer` 类，重点掌握编码（Encode）和解码（Decode）的核心逻辑
> - 实践：基于实现的 Tokenizer 类，将文本文件转换为编码后的二进制文件

拿到训练好的 vocab（词表）和 merges（合并规则）后，我们需要解决两个核心问题：如何将新句子转换为 Token ID？又如何将 ID 还原为原始文本？本节将围绕这两个问题，封装一个完整的字节级 BPE Tokenizer。
...
BPE 编码过程必须严格重现训练阶段的"演化历史"。训练时每一对合并规则产生的顺序被记录为 **Rank**：

- **Rank 0**：最早被合并的对（通常是最高频的）
- **Rank N**：最后被合并的对

> [!IMPORTANT]
> **BPE 铁律**：编码时，优先合并 Rank 最小的相邻对。Rank 越小，优先级越高。

### 编码流程推演

以单词 `target` 为例，初始字节序列为 `['t', 'a', 'r', 'g', 'e', 't']`。假设训练好的 merges 规则如下（按 Rank 升序排列）：
...
决策：虽然 `a,r` 在序列中更靠前，但 Rank 0 < Rank 1，优先合并 `e + t → et`。

**ROUND 2：新序列扫描**

当前序列：`['t', 'a', 'r', 'g', 'et']`（注意：`et` 已成为整体 Token）

存在的相邻对：
- `t, a` — 无规则
- `a, r` — 命中 Rank 1
- `r, g` — 无规则
- `g, et` — 命中 Rank 2（新产生的组合）

决策：Rank 1 < Rank 2，执行合并 `a + r → ar`。
...
决策：Rank 2 < Rank 3，执行合并 `g + et → get`。
...
3. **解码**：调用 `.decode('utf-8')` 还原为字符串

### 致命问题：不合法的字节流

> [!WARNING]
> 由于 BPE 基于统计，模型可能生成或被截断的"半个汉字"，导致 UTF-8 解码失败。

示例：汉字"牛"的 UTF-8 编码为 `b'\xe7\x89\x9b'`（3 字节）。若模型只生成了前两个字节 `b'\xe7\x89'` 就被截断（如达到 `max_len`），直接调用 `decode('utf-8')` 会抛出 `UnicodeDecodeError`，导致整个服务崩溃。
...
检测到非法字节时，用 `\ufffd`（Unicode 替换字符）替换该字节，而非中断解码，继续处理后续字节。
...
基于上述 `BPETokenizer` 类，可以将文本文件逐行编码为 ID 序列，再以二进制格式存储。核心思路：

1. 加载训练好的 `vocab` 和 `merges`（通常以 pickle 或 json 格式保存）
2. 实例化 `BPETokenizer`
3. 逐行读取文本文件，调用 `tokenizer.encode()` 获得 ID 列表
4. 将 ID 列表以二进制方式（如 `struct.pack` 或 `numpy.tofile`）写入文件

> [!TIP]
> 对于大文件，推荐使用 `encode_iterable` 方法配合文件迭代器，避免一次性将全部内容加载到内存。