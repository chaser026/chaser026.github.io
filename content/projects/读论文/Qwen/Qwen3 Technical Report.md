...
| 流程 |
|------|
| Plain Text<br>原始 Query 池<br>↓ Qwen2.5-72B 过滤<br>├─ 排除：不可验证 / 无需CoT / 开放生成<br>└─ 保留：难度适中、需要深度推理、可验证<br>↓ 领域均衡标注<br>↓ QwQ-32B 生成 N 个候选回答<br>↓ 6 条标准严格过滤<br>↓<br>精选数据集<br>├─ 小子集 → Cold Start SFT（最少量训练）<br>└─ 剩余 → Reasoning RL 阶段使用 |
